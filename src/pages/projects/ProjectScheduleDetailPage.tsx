...
import { PROJECT_OPTIONS, SCHEDULE_ITEMS } from "@/pages/projects/projectOpsMock";

export function ProjectScheduleDetailPage() {
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <Button asChild className="mt-4" variant="outline">
          <Link to={`/projects/${projectId}`}>프로젝트로 돌아가기</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
        ...
        <Link className="hover:text-foreground" to={`/projects/${project.id}`}>{project.name}</Link>
        <ChevronRight className="h-3.5 w-3.5" />
        <span>일정</span>
        <ChevronRight className="h-3.5 w-3.5" />
      ...
      <div className="flex items-center gap-2">
        <Button asChild variant="outline">
          <Link to={`/projects/${project.id}`}>목록으로</Link>
        </Button>
        <Button>일정 수정</Button>
      </div>
    </div>
