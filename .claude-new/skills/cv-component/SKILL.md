---
name: cv-component
description: 프론트엔드 컴포넌트 작성 규칙을 적용한다. packages/ui 프리미티브와 apps/web 도메인 컴포넌트의 2티어 구분, Props 설계, 합성 패턴, 렌더링 책임 경계 정리가 필요할 때 사용한다. 컴포넌트 추가/수정, Props 인터페이스 설계, 컴포넌트 분리/합성 작업에서 사용한다.
---
...
- packages/ui(프리미티브)와 apps/web(도메인) 컴포넌트의 책임을 명확히 분리하라.
...
| 티어 | 위치 | 역할 | 예시 |
|---|---|---|---|
| 프리미티브 | `packages/ui/src/` | 도메인 무관, props만으로 동작하는 UI 빌딩 블록 | Button, Input, Combobox, Dialog |
| 도메인 | `apps/web/src/` | 도메인 로직 포함, hook/store 사용 가능 | IssueCard, MemberSelector, ProjectSidebar |
...
## apps/web 도메인 컴포넌트 규칙

- packages/ui 프리미티브를 기반으로 조합/확장하라. 중복 커스텀 구현을 지양하라.
- hook 호출을 허용한다 (query hook, logic hook, store 접근).
- 단, API를 직접 호출(`fetch`, `axios`)하지 마라. 반드시 hook을 통해 접근하라.
- 도메인 컴포넌트가 도메인 무관해졌으면 packages/ui로 승격을 검토하라.
...
- 특화 라이브러리는 packages/ui에 넣지 마라. apps/web에서 직접 사용하거나 도메인 컴포넌트로 래핑하라.

### 컴포넌트 구현 시 체크 순서

1. shadcn/ui에 해당 컴포넌트가 있는가? → 있으면 그대로 사용하거나 확장.
2. 특화 라이브러리가 필요한가? (테이블, 차트, 에디터 등) → 해당 라이브러리 사용.
3. 둘 다 없으면 HeroUI 디자인을 참고해 새 프리미티브를 packages/ui에 작성.

## 적용 절차

1. 컴포넌트가 프리미티브(packages/ui)인지 도메인(apps/web)인지 결정하라.
2. Props 인터페이스를 먼저 정의하라 (최소 계약).
3. packages/ui 프리미티브를 기반으로 조합하라. 없으면 프리미티브 추가를 먼저 검토하라.
4. Props가 5개를 넘으면 compound/slot 패턴을 검토하라.
5. 300줄을 넘으면 하위 컴포넌트로 분리하라.
6. 접근성 기본 규칙을 점검하라.

## 빠른 체크리스트

- 컴포넌트가 올바른 티어(packages/ui vs apps/web)에 배치됐는가?
- packages/ui 컴포넌트에 API/store/도메인 타입 의존이 없는가?
- named export를 사용하는가?
- Props가 최소 계약만 포함하는가?
- 도메인 컴포넌트가 packages/ui 프리미티브를 기반으로 조합하는가?
