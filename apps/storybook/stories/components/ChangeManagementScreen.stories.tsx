...
function ChangeManagementScreenStory() {
    ...
    () =>
      activeItems.filter((item) => {
        ...
        const matchesState = queryState.state === "open"
          ? item.state !== "CLOSED" && item.crState !== "CLOSED" && item.crState !== "MERGED"
          : item.state === "CLOSED" || item.crState === "CLOSED" || item.crState === "MERGED";
  ...
  return (
    <ChangeManagementScreen
      ...
      isLoading={false}
      listData={{
        closedCount: activeItems.filter((item) => item.state === "CLOSED" || item.crState === "CLOSED" || item.crState === "MERGED").length,
        items: filteredItems,
        openCount: activeItems.filter((item) => item.state !== "CLOSED" && item.crState !== "CLOSED" && item.crState !== "MERGED").length,
