...
interface PartTemplateMappingStore {
  ...
  setMappings: (columnMappings: ColumnMappingModel[], relationMappings: RelationMappingModel[]) => void;
  resetMappings: () => void;
  ...
  persist(
    (set, get) => ({
      ...
      setMappingPreview: (preview) => {
        const columnMappings = preview.mapping.propertyMappings.map((mapping, index) => ({
          id: `cm-${index + 1}`,
          ...
          approved: mapping.isExtended || (mapping.confidence ?? 0) >= AUTO_APPROVE_CONFIDENCE_THRESHOLD,
          isExtended: mapping.isExtended ?? false,
        }));

        const relationMappings = preview.mapping.relationMappings.map((mapping, index) => ({
          id: `rm-${index + 1}`,
        ...
        const initialColumnMappings = columnMappings.map((mapping) => ({ ...mapping }));
        const initialRelationMappings = relationMappings.map(cloneRelationMappingModel);

        set({
          mappingHeaders: preview.headers,
          mappingSampleRows: preview.sampleRows,
          columnMappings: initialColumnMappings.map((mapping) => ({ ...mapping })),
          relationMappings: initialRelationMappings.map(cloneRelationMappingModel),
      ...
      resetMappings: () =>
        set((state) => ({
          columnMappings: state.initialColumnMappings.map((mapping) => ({ ...mapping })),
          relationMappings: state.initialRelationMappings.map(cloneRelationMappingModel),
        })),
    ...
    {
      name: "web2-part-template-mapping-store",
      version: 1,
