...
interface OrganizationSettingsStoreState {
  ...
  setSecurity: (next: Partial<Omit<OrganizationSecuritySettings, "allowedIps">>) => void;
...
export const useOrganizationSettingsStore = create<OrganizationSettingsStoreState>()(
  persist(
    (set) => ({
      ...
      removeAllowedIp: (id) =>
        set((state) => ({
          security: {
            ...state.security,
            allowedIps: state.security.allowedIps.filter((entry) => entry.id !== id),
          },
        })),
      reset: () =>
        set({
    ...
    {
      name: "fabbit-web2-organization-settings",
      version: 1,
      partialize: (state) => ({
