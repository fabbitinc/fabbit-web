...
export function useArchiveProjectAction(projectId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    ...projectDetailMutations.archive(projectId),
    mutationKey: ["project-detail", projectId, "archive-project-action"],
    onSuccess: async () => {
    ...
    onError: (error) => {
      toast.error(extractApiError(error, "프로젝트 보관에 실패했습니다."));
    },
  });
}
