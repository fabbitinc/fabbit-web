...
export function useDeletePartDrawingItemAction(partId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationKey: ["parts", partId, "delete-part-drawing-item-action"],
    mutationFn: (drawingId: string) => deletePartDrawing(partId, drawingId),
    onSuccess: async () => {
