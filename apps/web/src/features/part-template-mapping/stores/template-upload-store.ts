...
interface TemplateUploadStore {
  ...
  updateUploadedFile: (
    fileId: string,
    updates: Partial<UploadedTemplateFileModel>,
  ) => void;
...
export const useTemplateUploadStore = create<TemplateUploadStore>()(
  persist(
    (set) => ({
      ...
      setPrimaryUploadCompleted: (isCompleted) =>
        set({ isPrimaryUploadCompleted: isCompleted }),
      resetUploadState: () => set(INITIAL_TEMPLATE_UPLOAD_STATE),
    }),
    {
      name: "web-part-template-upload-store",
      version: 1,
