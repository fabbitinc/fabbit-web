...
/** 프로젝트 변경 반영 생성 */
export async function createProjectChange(projectId: string, request: CreateChangeRequest): Promise<ChangeDto> {
  const response = await apiClient.post<ApiChangeResponse>(`/api/v1/projects/${projectId}/changes`, request);
  return mapChange(response.data);
}

/** 프로젝트 변경 반영 목록 조회 */
export async function getProjectChanges(projectId: string): Promise<ChangeListResponse> {
  const response = await apiClient.get<unknown>(`/api/v1/projects/${projectId}/changes`);
...
/** 프로젝트 변경 반영 상세 조회 */
export async function getProjectChange(projectId: string, changeNumber: string): Promise<ChangeDto> {
  const response = await apiClient.get<ApiChangeResponse>(`/api/v1/projects/${projectId}/changes/${changeNumber}`);
  return mapChange(response.data);
}
...
/** 변경 반영 타임라인 조회 */
export async function getChangeTimeline(projectId: string, changeNumber: string): Promise<IssueTimelineResponse> {
  const response = await apiClient.get<ApiTimelineResponse>(`/api/v1/projects/${projectId}/changes/${changeNumber}/timeline`);
  return {
...
/** 변경 반영 담당자 동기화 (PUT) */
export async function syncChangeAssignees(projectId: string, changeNumber: string, userIds: string[]): Promise<void> {
  await apiClient.put(`/api/v1/projects/${projectId}/changes/${changeNumber}/assignees`, {
    user_ids: userIds,
  });
}

/** 변경 반영 검토자 동기화 (PUT) */
export async function syncChangeReviewers(projectId: string, changeNumber: string, userIds: string[]): Promise<void> {
  await apiClient.put(`/api/v1/projects/${projectId}/changes/${changeNumber}/reviewers`, {
    user_ids: userIds,
  });
}

/** 변경 반영용 프로젝트 라벨 목록 조회 */
export async function getChangeLabels(projectId: string): Promise<IssueLabelDto[]> {
  const response = await apiClient.get<ApiLabelListResponse>(`/api/v1/projects/${projectId}/labels`);
  return (response.data.items ?? [])
    .map(mapChangeLabel)
    .filter((label): label is IssueLabelDto => label !== null) as IssueLabelDto[];
}

/** 변경 반영 라벨 동기화 */
export async function syncChangeLabels(projectId: string, changeNumber: string, labelIds: string[]): Promise<void> {
  await apiClient.put(`/api/v1/projects/${projectId}/changes/${changeNumber}/labels`, {
    label_ids: labelIds,
  });
}

/** 변경 반영 부품 동기화 (PUT) */
export async function syncChangeParts(projectId: string, changeNumber: string, partIds: string[]): Promise<void> {
  await apiClient.put(`/api/v1/projects/${projectId}/changes/${changeNumber}/parts`, {
    part_ids: partIds,
  });
}

/** 변경 반영에 파일 첨부 */
export async function attachChangeFiles(projectId: string, changeNumber: string, fileIds: string[]): Promise<void> {
  await apiClient.post(`/api/v1/projects/${projectId}/changes/${changeNumber}/files`, {
    file_ids: fileIds,
  });
}

/** 변경 반영에서 파일 삭제 */
export async function deleteChangeFile(projectId: string, changeNumber: string, fileId: string): Promise<void> {
  await apiClient.delete(`/api/v1/projects/${projectId}/changes/${changeNumber}/files/${fileId}`);
}
...
/** 변경 반영 댓글 삭제 */
export async function deleteChangeComment(projectId: string, changeNumber: string, commentId: string): Promise<void> {
  await apiClient.delete(`/api/v1/projects/${projectId}/changes/${changeNumber}/comments/${commentId}`);
}
