...
import type { PartsTemplateMappingBoardCard } from "./types";
import { getPropertyDisplayName } from "./types";

interface PartsTemplateMappingDragOverlayProps {
  card: PartsTemplateMappingBoardCard;
  propertyLabelByName: Record<string, string>;
}

export function PartsTemplateMappingDragOverlay({
  card,
  propertyLabelByName,
}: PartsTemplateMappingDragOverlayProps) {
  const mapped = card.targetProperty
    ? getPropertyDisplayName(propertyLabelByName, card.targetProperty)
    : card.relProperty
      ? getPropertyDisplayName(propertyLabelByName, card.relProperty)
      : undefined;
    ...
    <div className="flex w-[300px] items-center gap-2 rounded-md border border-blue-300 bg-white px-2.5 py-2 shadow-lg">
      ...
      {mapped ? (
        <>
          ...
          <span className="truncate text-[10px] text-gray-500">{mapped}</span>
        </>
      ) : null}
      ...
      <div className="ml-auto flex shrink-0 items-center">
        {card.approved ? <Check className="size-3.5 text-emerald-500" /> : null}
      </div>
    </div>
  );
}
