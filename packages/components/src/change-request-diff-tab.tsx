...
export interface ChangeRequestDiffPartSnapshot {
  part_number: string;
  name: string;
  revision: string;
...
export interface ChangeRequestDiffItem {
  ...
  before: ChangeRequestDiffPartSnapshot | null;
  after: ChangeRequestDiffPartSnapshot | null;
}

export interface ChangeRequestDiffChanges {
  items: ChangeRequestDiffItem[];
}

export interface ChangeRequestDiffTabProps {
  changes?: ChangeRequestDiffChanges;
  emptyMessage?: string;
}

const FIELD_LABELS: { key: keyof ChangeRequestDiffPartSnapshot; label: string }[] = [
...
const TYPE_LABEL: Record<ChangeRequestDiffItemType, string> = {
  modified: "수정",
  added: "추가",
  removed: "삭제",
};

const TYPE_BADGE_CLASS: Record<ChangeRequestDiffItemType, string> = {
...
const DEFAULT_CHANGES: ChangeRequestDiffChanges = {
  items: [
    {
      id: "ci1",
      partId: "p1",
...
function formatValue(key: keyof ChangeRequestDiffPartSnapshot, value: unknown): string {
  if (value == null) {
    return "—";
  }
...
function PartDiffCard({ item }: { item: ChangeRequestDiffItem }) {
...
export function ChangeRequestDiffTab({
  ...
  const counts = changes.items.reduce(
    ...
    { modified: 0, added: 0, removed: 0 } as Record<ChangeRequestDiffItemType, number>,
  );
