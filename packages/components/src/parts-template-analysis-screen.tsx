...
export interface PartsTemplateAnalysisExample {
  title: string;
  description: string;
...
export interface PartsTemplateAnalysisScreenProps {
  description?: string;
  examples?: PartsTemplateAnalysisExample[];
  isUploading?: boolean;
  accept?: string;
  onBackClick: () => void;
...
const DEFAULT_EXAMPLES: PartsTemplateAnalysisExample[] = [
  {
    title: "부품 목록 파일",
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="app-panel rounded-lg p-6">
        ...
        <div
          className={getDropzoneClassName(isUploading, isDragOver)}
          onClick={() => {
            if (!isUploading) {
        ...
        >
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm font-medium text-foreground">
              {isUploading ? "파일을 업로드하는 중입니다" : "파일을 드래그하거나 클릭해서 선택하세요"}
            </p>
...
function TemplateExampleCard({ title, description, rows }: PartsTemplateAnalysisExample) {
